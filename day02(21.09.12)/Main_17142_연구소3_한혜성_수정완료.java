...
public class Main_한혜성 {
	...
	public static void main(String[] args) throws Exception {
		...
		indexch = new Choice[M];
		  ...
		  if (zerocount == 0) {	//빈칸 0 이면 퍼뜨릴 곳 없다!
	            System.out.println(0);
	        } else {
	            combi(0, 0);
	            System.out.println(minmin == Integer.MAX_VALUE ? -1 : minmin);
	        }
	...
	public static void go_bfs(int zerocount) {
		...
		while(!que.isEmpty()) {
			...
			for (int d = 0; d < 4; d++) {
				...
				if(chch[nx][ny] || map[nx][ny] == 1) continue;
